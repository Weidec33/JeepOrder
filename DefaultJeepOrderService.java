...
public class DefaultJeepOrderService implements JeepOrderService {
	...
	@Transactional
	@Override
	public Order createOrder(OrderRequest orderReqest) {
		Customer customer = getCustomer(orderReqest);
		  Jeep jeep = getModel(orderReqest);
		  Color color = getColor(orderReqest);
		  Engine engine = getEngine(orderReqest);
		  Tire tire = getTire(orderReqest);
		  List<Option> options = getOption(orderReqest);
